...
function amountInfoInit() {
  ...
  $('<span class="titlefont4">  TWD</span>').appendTo(amount_info_price);
  ...
  $('<span class="titlefont4">  TWD</span>').appendTo(amount_info_discount_price);
  ...
  $('<span class="titlefont4">  TWD</span>').appendTo(amount_info_payment_amount);
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < selectedProductsInfo.length; i++) {
        totalQuantity += parseInt(selectedProductsInfo[i].quantity)

        // #40038 
        if( totalQuantity == 3 && $('#discountText').length == 0 && location.pathname == '/hk/vgrphk/'){
            $('#amount_info_payment_amount').append('<div id="discountText" style="color: red; font-weight: 900;">恭喜您符合滿額折扣優惠資格，支付金額已折1,440</div>')
        }
        // #32575 增加折扣紅字
        // #40830 取消紅字折扣
        // else if( totalQuantity == 3 && $('#discountText').length == 0){
        //   $('#amount_info_payment_amount').append('<div id="discountText" style="color: red; font-weight: 900;">恭喜您符合滿額折扣優惠資格，支付金額已折1,140</div>')
        // }
        // else if (totalQuantity < 3 || totalQuantity > 3) {
        //   $('#discountText').remove();
        // }
